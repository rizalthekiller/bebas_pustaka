...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'buat_surat') {
    ...
    // Validasi nomor surat
    $tahun = date('Y', strtotime($tanggal_surat));
    $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM tb_surat WHERE nomor_surat = ? AND YEAR(tanggal_surat) = ? AND id != ?");
    $stmt->execute([$nomor_surat, $tahun, $id_surat]);
    $result = $stmt->fetch();
                                            ...
                                            <?php foreach ($surat_list as $surat): ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        <a href="surat.php?action=kirim_email&id=<?php echo $surat['id']; ?>" class="btn btn-info btn-sm">
                                                            <i class="fas fa-envelope me-2"></i>Kirim Email
                                                        </a>
                                                    </td>
                                                </tr>
